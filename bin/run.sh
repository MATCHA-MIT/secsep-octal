...
# usage
function usage {
    echo "Usage: $0 --bench <input assembly> --interface <function interfaces> [--inter <inter product name>] [--out-dir <output directory>]"
    exit 1
}
...
if [ -z "$INTERFACE" ]; then
    echo "Please specify the interface name with --interface"
    usage
fi
...
time dune exec infer_single   1>$OUT_DIR/$INTER_NAME.single_infer.log 2>&1 -- -name $INTER_NAME -asm $BENCH -interface $INTERFACE
if [ $? -ne 0 ]; then
