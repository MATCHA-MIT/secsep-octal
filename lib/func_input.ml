...
module FuncInput = struct
  ...
  let fix_stack_push_slot
      (func: Isa.func) 
      (stack_spill_info: StackSpillInfo.t) 
      (mem_type: MemType.t) :  StackSpillInfo.t * MemType.t =
    ...
    let fix_stack 
        ...
        (FuncInputEntry.t MemType.mem_slot) list * (int64 * int64) list =
      let rec helper 
        ...
        | [] -> 
          List.init num_push (
            fun i -> 
              ...
              let off_l = Int64.sub off 8L in
              ((SingleEntryType.SingleConst off_l, SingleEntryType.SingleConst off), MemRange.RangeConst [], (SingleEntryType.SingleTop, None)),
              (off_l, off)
          ) |> List.split
        | ((SingleConst l, SingleConst r), range, entry) :: tl ->
          ...
          if r = start_off && l = off_minus_eight then 
            let new_tl, new_spill = helper (num_push - 1, off_minus_eight) tl in
            ((SingleConst l, SingleConst r), range, entry) :: new_tl, (l, r) :: new_spill
            (* ((SingleConst l, SingleConst r), range, entry) :: (helper (num_push - 1, off_minus_eight) tl) *)
          else if r <= off_minus_eight then
            let new_slot_list, new_spill = helper (num_push - 1, off_minus_eight) slot_list in
            ((SingleConst off_minus_eight, SingleConst start_off), RangeConst [], (SingleTop, None)) :: new_slot_list,
            (off_minus_eight, start_off) :: new_spill
      ...
      let new_slots, new_spill = helper (num_push, 0L) (List.tl rev_stack_slots) in
      List.rev ((List.hd rev_stack_slots) :: new_slots), new_spill
    in
    List.fold_left_map (
      fun acc (ptr, slots) ->
        if fst ptr = Isa.rsp_idx then
          let new_slots, new_spill = fix_stack slots in
          Int64PairSet.union acc (Int64PairSet.of_list new_spill), (ptr, new_slots)
        else acc, (ptr, slots)
    ) stack_spill_info mem_type
  ...
  let update_func_input 
    ...
    let helper (func_interface: entry_t) : entry_t = 
      ...
      | None -> func_interface
      | Some func ->
        let new_spill, mem_type = fix_stack_push_slot func func_interface.stack_spill_info func_interface.mem_type in
        ...
        { func_interface with 
          mem_type = 
            List.fold_left (fun x f -> f x) 
              mem_type update_mem_helper_list;
          stack_spill_info = new_spill;
        }
    in
