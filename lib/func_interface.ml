...
module FuncInterface (Entry: EntryType) = struct
  ...
  let add_mem_var_map
      ...
      var_map_set_t * ((Isa.imm_var_id * (read_hint_t list)) list) =
    let helper_inner
        (acc: var_map_set_t)
        (entry: read_hint_t) :
        var_map_set_t * read_hint_t =
      let single_var_map, single_var_set, var_map = acc in
      match entry with
      | Untrans (c_off, c_range, c_entry) ->
        if MemOffset.is_val single_var_set c_off && MemRange.is_val single_var_set c_range then
          let m_off = MemOffset.repl_local_var single_var_map c_off in
          let m_range = MemRange.repl_local_var single_var_map c_range in
    ...
    let helper_outer
        ...
        var_map_set_t * (Isa.imm_var_id * (read_hint_t list)) =
      let _, single_var_set, _ = acc in
    ...
    in
    let rec helper
        (acc: var_map_set_t) (child_mem: (Isa.imm_var_id * (read_hint_t list)) list) :
        var_map_set_t * ((Isa.imm_var_id * (read_hint_t list)) list) =
      let single_var_map, _, _ = acc in
      let l = List.length single_var_map in
      let (single_var_map, single_var_set, var_map), child_mem =
        List.fold_left_map helper_outer acc child_mem
      in
      if List.length single_var_map > l then helper (single_var_map, single_var_set, var_map) child_mem
      else (single_var_map, single_var_set, var_map), child_mem
    in
    ...
    in
    helper var_map_set child_mem_helper
  ...
  let set_one_entry
      ...
      (MemOffset.t * MemRange.t * entry_t) * (Constraint.t list) =
    ...
    let p_range, out_range_constraint =
      if MemRange.is_val single_var_set c_out_range then
        let m_out_range = MemRange.repl_local_var single_var_map c_out_range in
        MemRange.merge smt_ctx p_range m_out_range, []
      else p_range, [ Constraint.Unknown (SingleTop, SingleTop) ]
    in
    if Entry.cmp c_in_entry c_out_entry = 0 then 
      (* NOTE: c_in_entry is before repl, so it must be var and eq means it is unchanged!!! *)
  ...
  let set_part_mem
    ...
    let rec helper
      ...
      | _, [], [] -> finished_parent_entry @ parent_entry, constraint_list
      | _, (Untrans _) :: read_hint, _ :: write_mem ->
        helper finished_parent_entry ((Unknown (SingleTop, SingleTop)) :: constraint_list) parent_entry read_hint write_mem
      | _, (Unmapped off) :: read_hint, _ :: write_mem ->
        helper finished_parent_entry ((Unknown off) :: constraint_list) parent_entry read_hint write_mem
      | (p_off, p_range, p_entry) :: parent_entry_tl,
        ...
        if MemOffset.cmp p_off mp_off = 0 then
          let new_entry, new_constraints = set_one_entry smt_ctx var_map_set (p_off, p_range, p_entry) (is_full, m_in_range, c_in_entry) write_entry in
          helper (new_entry :: finished_parent_entry) (new_constraints @ constraint_list) parent_entry_tl read_hint_tl write_mem_tl
        else
          helper ((p_off, p_range, p_entry) :: finished_parent_entry) constraint_list parent_entry_tl read_hint write_mem
      | _ -> func_interface_error "write_one_ptr_entries: unexpected case"
    in
    let rev_parent_entry, constraints = helper [] [] parent_entry read_hint write_mem in
    List.rev rev_parent_entry, constraints
  ...
  let set_mem_type
    ...
    let helper
        ...
        MemType.t * (Constraint.t list) =
      let p_mem, constraint_list = acc in
      ...
      if ptr = ptr2 then
        if SingleExp.SingleVarSet.mem ptr single_var_set then
          let p_base = SingleExp.repl_local_var single_var_map (SingleVar ptr) in
          ...
          | Some p_ptr ->
            ...
            | Some (new_part_mem, new_constraints) ->
              List.map (
                fun (x, part_mem) -> 
                  if x = p_ptr then x, new_part_mem else x, part_mem
              ) p_mem, new_constraints @ constraint_list
            | None -> p_mem, (Unknown (SingleTop, SingleTop)) :: constraint_list
            end
          | None -> p_mem, (Unknown (p_base, p_base)) :: constraint_list
        else
          p_mem, (Unknown (SingleTop, SingleTop)) :: constraint_list
      else
        func_interface_error "set_mem_type: read_hint and write_mem ptr does not match %d %d\n" ptr ptr2
    in
    List.fold_left2 helper (parent_mem, []) read_hint write_mem
  ...
  let func_call
      ...
      RegType.t * MemType.t * (Constraint.t list) =
    ...
    in
    let (single_var_map, single_var_set, var_map), mem_read_hint =
      add_mem_var_map smt_ctx (single_var_map, single_var_set, var_map) child_mem parent_mem
    in
    let reg_type = set_reg_type global_var_set var_map child_out_reg in
    let mem_type, constraint_list = 
      set_mem_type smt_ctx (single_var_map, single_var_set, var_map) parent_mem mem_read_hint child_out_mem in
    reg_type, mem_type, constraint_list
