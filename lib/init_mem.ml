...
module InitMem = struct
  ...
  let init_addr_exp (p: Isa.program) (init_code_type: CodeType.t) : (TypeFullExp.t * int64) list =
    let helper_0 
        (acc: (TypeFullExp.t * int64) list * CodeType.state_type * CondType.t list) 
        (inst: Isa.instruction) : 
        (TypeFullExp.t * int64) list * CodeType.state_type * CondType.t list =
      let addr_list, state, cond_list = acc in
      let get_addr_type (op: Isa.operand) : (TypeFullExp.t * int64) option =
        match op with
    ...
    in
    let helper 
        (acc: (TypeFullExp.t * int64) list * CondType.t list)
        (block_type: CodeType.block_type)
        (block: Isa.basic_block) : 
        (TypeFullExp.t * int64) list * CondType.t list =
  ...
  let get_addr_range (addr_exp_list: (TypeFullExp.t * int64) list) : (SingleExp.t * SingleExp.t) list =
    let helper (addr_exp: TypeFullExp.t * int64) : SingleExp.t * SingleExp.t = 
  ...
  let get_base (ptr_list: SingleExp.SingleVarSet.t) (addr_list: (TypeFullExp.t * int64) list) : Isa.imm_var_id list =
