...
module InitStack = struct
  ...
  module OffsetSet = Set.Make(Int64)

  let add_stack_slots (stack_slots: OffsetSet.t) (offset: int64) (operand: Isa.operand) : OffsetSet.t =
    match operand with
    | LdOp (None, Some Isa.RSP, None, None, _)
    | StOp (None, Some Isa.RSP, None, None, _) -> OffsetSet.add offset stack_slots
    | LdOp (Some (ImmNum x), Some Isa.RSP, None, None, _)
    | StOp (Some (ImmNum x), Some Isa.RSP, None, None, _) -> OffsetSet.add (Int64.add offset x) stack_slots
    | _ -> stack_slots

  let find_offset_helper (offset_list: (int64 option) list) (stack_slots: OffsetSet.t) (p: Isa.program) : (int64 option) list * OffsetSet.t =
    let helper (acc: (int64 option) list * OffsetSet.t * int) (block: Isa.basic_block) : (int64 option) list * OffsetSet.t * int =
      let helper2 (acc2: (int64 option) list * OffsetSet.t * int64)  (inst: Isa.instruction) : (int64 option) list * OffsetSet.t * int64 =
        let old_offset_list, stack_slots, offset = acc2 in
        match inst with
        (* Insts that affect rsp offset *)
        | Push _ -> (old_offset_list, stack_slots, Int64.sub offset 8L)
        | Pop _ -> (old_offset_list, stack_slots, Int64.add offset 8L)
        | Add (RegOp dst, RegOp r, ImmOp (ImmNum i))
        | Add (RegOp dst, ImmOp (ImmNum i), RegOp r) -> (* NOTE: Maybe remove the second case *)
          if dst = Isa.RSP && r = Isa.RSP then (old_offset_list, stack_slots, Int64.add offset i) else (old_offset_list, stack_slots, offset)
        | Sub (RegOp dst, RegOp r, ImmOp (ImmNum i)) ->
          if dst = Isa.RSP && r = Isa.RSP then (old_offset_list, stack_slots, Int64.sub offset i) else (old_offset_list, stack_slots, offset)
  ...
  let offset_solved (offset_list: (int64 option) list) : bool =
    let helper (acc: bool) (offset: int64 option) =
      match offset with
      | Some _ -> acc
      | None -> false
    in
  ...
  let find_offset (offset_list: (int64 option) list) (stack_slots: OffsetSet.t) (p: Isa.program) (num_iter: int) : (int64 option) list * OffsetSet.t =
  ...
  let update_offset (p: Isa.program) : Isa.program * (int64 list) =
    let init_offset_list = List.mapi (fun i _ -> if i = 0 then Some 0L else None) p.bbs in
    let offset_list, stack_slots = find_offset init_offset_list OffsetSet.empty p 1 in
    let helper (new_offset: int64 option) (block: Isa.basic_block) : Isa.basic_block =
      match new_offset with
      | Some o -> { block with rsp_offset = o }
      | None -> init_stack_error ("offset not initialized " ^ block.label)
    in
    ({ p with bbs = List.map2 helper offset_list p.bbs }, OffsetSet.to_list stack_slots)
