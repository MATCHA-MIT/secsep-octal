...
include SingleExpBasic
  ...
  let rec eval_t (e: t) : t list list =
    ...
    | SingleITE ((cond, cl, cr), l, r) ->
      (* We eval cond in reorder_ite, so we skip eval here for optimization *)
  ...
  let reorder_ite (e: t) : t =
    ...
    in
    let rec helper 
        (true_cond_set: SingleCondSet.t) (false_cond_set: SingleCondSet.t) 
        (e: t) : t =
      match e with
      | SingleTop | SingleConst _ | SingleVar _ -> e
      | SingleBExp (op, e1, e2) ->
        let e1 = helper true_cond_set false_cond_set e1 in
        let e2 = helper true_cond_set false_cond_set e2 in
        begin match e1, e2 with
        | SingleITE (cond1, cl1, cr1), _ ->
          SingleITE (cond1, helper true_cond_set false_cond_set (SingleBExp (op, cl1, e2)), helper true_cond_set false_cond_set (SingleBExp (op, cr1, e2)))
        | _, SingleITE (cond2, cl2, cr2) ->
          SingleITE (cond2, helper true_cond_set false_cond_set (SingleBExp (op, e1, cl2)), helper true_cond_set false_cond_set (SingleBExp (op, e1, cr2)))
        | _ -> e
        end
      | SingleUExp (op, e0) ->
        let e0 = helper true_cond_set false_cond_set e0 in
        begin match e0 with
        | SingleITE (cond, cl, cr) ->
          SingleITE (cond, helper true_cond_set false_cond_set (SingleUExp (op, cl)), helper true_cond_set false_cond_set (SingleUExp (op, cr)))
        | _ -> e
        end
      | SingleITE (cond, cl, cr) ->
        ...
        if cond_has_top_helper cond then
          ...
          SingleITE (
            cond, 
            helper (SingleCondSet.add cond true_cond_set) false_cond_set cl, 
            helper true_cond_set (SingleCondSet.add cond false_cond_set) cr
          )
      in
      helper SingleCondSet.empty SingleCondSet.empty e
